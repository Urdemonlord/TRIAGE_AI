...
export default function PatientCheckPageWithSupabase() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call NextJS API route yang akan:
      // 1. Call AI Service untuk predictions
      // 2. Save ke Supabase
      const response = await fetch('/api/triage', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Supabase will use the user's session token automatically
        },
        body: JSON.stringify({
      ...
      setSuccessMessage('✅ Analisis berhasil! Data tersimpan di database.');
      setComplaint('');

      // Navigate to result page dengan session_id dari Supabase
      setTimeout(() => {
        router.push(`/patient/result?id=${result.session_id}`);
      }, 1500);
    } catch (err: any) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="text-sm text-gray-600">
              ...
              {isAuthenticated && <span className="ml-4 text-success-600">✓ Login</span>}
            </div>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {!isAuthenticated && (
          <div className="bg-warning-50 border border-warning-200 rounded-lg p-4 mb-8">
            <p className="text-sm text-warning-800">
              <strong>Perhatian:</strong> Anda belum login.{' '}
              <Link href="/login" className="underline font-medium text-warning-900">
                Silakan login terlebih dahulu
              </Link>
              {' '}atau{' '}
              <Link href="/signup" className="underline font-medium text-warning-900">
                buat akun baru
              </Link>
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              type="submit"
              disabled={loading || !complaint.trim() || !isAuthenticated}
